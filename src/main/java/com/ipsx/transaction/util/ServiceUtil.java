package com.ipsx.transaction.util;
...
public class ServiceUtil {
    ...
    public static Map<Integer, List<Integer>> shiftBlanks(Transaction transaction, int colNum, long excess, int swapVal) {
        ...
        for(int i = 0; i < Constants.COMBINED_COLUMN_LENGTH; i++) {
            if(swapVal == 0){
    ...
    private static long performSwap(int i, List<Integer> thisCol, List<Integer> nextCol, long excess, int swapVal) {
        if ( (i % Constants.COLUMN_LENGTH_OF_EACH_TICKET == 0)
                && (i + Constants.COLUMN_LENGTH_OF_EACH_TICKET) < Constants.COMBINED_COLUMN_LENGTH
                && (nextCol.subList(i, (i + Constants.COLUMN_LENGTH_OF_EACH_TICKET))).stream().filter(val -> val == -1).count() >= 2) {
